# Top 2000s and Now Playlist Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `scripts/fill-2000s-playlist.mjs` — a single re-runnable script that creates the "Top 2000s and Now" playlist and fills it with ~150 songs from 2000–2025, importing missing songs from Spotify.

**Architecture:** Single `.mjs` script following the exact same pattern as `scripts/fill-top100-playlist.mjs`. On first run it creates the playlist and fills it; on re-runs it detects the playlist and skips songs already in it. Songs absent from the Supabase catalog are searched on Spotify, upserted into `songs`, and then added to `playlist_songs`.

**Tech Stack:** Node.js (ES modules), `@supabase/supabase-js`, `ws`, Spotify Web API (Client Credentials + Search), native `fetch`.
...
| File | Action |
|------|--------|
| `scripts/fill-2000s-playlist.mjs` | **Create** — complete new script |

No web app files are touched.

---

### Task 1: Scaffold — env, Supabase client, normalize, REFERENCE list

**Files:**
- Create: `scripts/fill-2000s-playlist.mjs`

- [ ] **Step 1: Create the file with all scaffold code and the 180-song reference list**

Create `scripts/fill-2000s-playlist.mjs` with this exact content:
...
```bash
node --check scripts/fill-2000s-playlist.mjs
```
...
```bash
git add scripts/fill-2000s-playlist.mjs
git commit -m "feat: scaffold fill-2000s-playlist with env, Supabase client, and reference list"
```

---

### Task 2: Spotify helpers

**Files:**
- Modify: `scripts/fill-2000s-playlist.mjs`

Append three functions after the REFERENCE array. These are identical in logic to `scripts/fill-top100-playlist.mjs`.

- [ ] **Step 1: Append the Spotify helpers to the file**

Append to `scripts/fill-2000s-playlist.mjs` (after the closing `];` of REFERENCE):
...
```bash
git add scripts/fill-2000s-playlist.mjs
git commit -m "feat: add Spotify helpers to fill-2000s-playlist"
```
...
Append the complete `main()` function and the entry point call. The key difference from `fill-top100-playlist.mjs`: this script **creates** the playlist if it doesn't exist (rather than aborting).

- [ ] **Step 1: Append main() and the entry call**

Append to `scripts/fill-2000s-playlist.mjs` (after the `fetchPreviewFromEmbed` closing brace):
...
```bash
git add scripts/fill-2000s-playlist.mjs
git commit -m "feat: add main function to fill-2000s-playlist"
```

---

### Task 4: Run the script and commit final state

**Files:**
- No changes — this task just runs the script and records the result.
...
Looking up "Top 2000s and Now" playlist... not found. Creating it.
 (0 songs already in it).
Loading catalog... NNN songs.

Processing 180 reference songs...

✓ from catalog Crazy in Love — Beyonce
✓ imported     Hey Ya! — OutKast  (spotify:track:...)
✗ no preview   Some Song — Some Artist
...

─────────────────────────────────────────
Already in playlist:      0
Added from catalog:      NN
Imported from Spotify:  NNN
Skipped (no preview):   NNN
Not found on Spotify:   NNN
Errors:                   0
─────────────────────────────────────────
"Top 2000s and Now" playlist now has NNN songs.
Edit it at: /admin/playlists/<uuid>
```

If `Errors: 0` and the playlist has songs, the run succeeded.

- [ ] **Step 2: If there are errors, investigate**

Common causes:
- `Missing SPOTIFY_CLIENT_ID` → check `.env.local` has `SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET`
- `Spotify token request failed: 400` → credentials are wrong
- Supabase errors → check `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`

Re-run is safe — the script is fully idempotent.

- [ ] **Step 3: Commit**

```bash
git add scripts/fill-2000s-playlist.mjs
git commit -m "feat: add fill-2000s-playlist script — creates and fills Top 2000s and Now playlist"
```
